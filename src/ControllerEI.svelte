<script>
  ...
  const gravity = writable('south');
  const textY = writable(20); // Nueva variable para la posición Y

  const brightnessLevel = writable(0);
  const contrastLevel = writable(0);
  const saturationLevel = writable(0);

  // Lista de fuentes de Cloudinary
  const cloudinaryFonts = ['Arial', 'Times', 'Times New Roman', 'Roboto'];
...
<div class="flex flex-col w-full space-y-6 p-6 bg-gray-50 rounded-md shadow-md overflow-y-scroll">
  ...
  <div class="flex items-center justify-between">
    ...
    <input 
      ...
      on:change={() => updateProperty('updateText', { applyText: $applyText, textContent: $textContent, textColor: $textColor, textSize: $textSize, textFont: $textFont, gravity: $gravity, textY: $textY })}
      class="toggle-checkbox hidden"
      id="toggle-text"
    />
    <label 
  ...
  {#if $applyText}
    <div class="flex flex-col space-y-4 pl-4 border-l-2 border-gray-200">
      ...
      <div class="flex items-center space-x-2">
        <label for="text-y" class="text-gray-700 font-medium">Posición Y:</label>
        <input 
          type="range" 
          id="text-y" 
          min="0" 
  ...
  {/if}
  <div class="flex items-center space-x-2">
    <label for="brightness" class="text-gray-700 font-medium">Brillo:</label>
    <input 
      type="range" 
      id="brightness" 
      min="-100" 
      ...
      on:change={() => updateProperty('updateBrightness', { brightnessLevel: $brightnessLevel })}
      class="w-full max-w-xs accent-blue-500"
    />
  </div>    
  <div class="flex items-center space-x-2">
    <label for="saturation" class="text-gray-700 font-medium">Saturación:</label>
    <input 
      type="range" 
      id="saturation" 
      min="0" 
      max="200" 
      bind:value={$saturationLevel} 
      on:change={() => updateProperty('updateSaturation', { saturationLevel: $saturationLevel })}
      class="w-full max-w-xs accent-blue-500"
    />
  </div>
  
  <div class="flex items-center space-x-2">
    <label for="contrast" class="text-gray-700 font-medium">Contraste:</label>
    <input 
      type="range" 
      id="contrast" 
      min="0" 
      max="100" 
      bind:value={$contrastLevel} 
      on:change={() => updateProperty('updateOpacity', { contrastLevel: $contrastLevel })}
      class="w-full max-w-xs accent-blue-500"
    />
  </div>
  <!-- Botón de descarga -->
