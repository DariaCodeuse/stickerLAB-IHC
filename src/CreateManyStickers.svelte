<script lang="ts">
  ...
  const title = "Crea muchos";
  const boldTitle = "STICKERS";
  const subtitle = "Sube varias imágenes, quítales el fondo y agrégales bordes. Acomódalas automáticamente o manualmente en una hoja tamaño carta para imprimir tus stickers.";
  ...
  onMount(() => {
    ...
    dropzone.on("error", (file, response) => {
      console.log("Error al subir:", response);
    });
  });
</script>

<!-- Contenido principal con condicional para mostrar StepEdit o Dropzone -->
  ...
  {#if $imageStatus === ImageStatus.READY || $imageStatus === ImageStatus.UPLOADING}
    <div class="w-full max-w-2xl px-4 text-center">
      ...
      <form
        ...
        action="https://api.cloudinary.com/v1_1/dr4htmwol/image/upload"
      >
        <svg 
          xmlns="http://www.w3.org/2000/svg" 
          viewBox="0 0 24 24" 
          fill="currentColor" 
          class="h-12 w-12 text-gray-500 group-hover:text-blue-400 transition duration-300"
        >
        ...
        <p class="text-gray-700 font-medium group-hover:text-blue-600 transition duration-300">
          Seleccionar Archivos PNG/JPG
        </p>
        <p class="text-sm italic text-gray-500 group-hover:text-blue-400 transition duration-300">
